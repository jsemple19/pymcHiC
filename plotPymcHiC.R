...
### read in data
FragAlin<-read.csv(paste0(path, expName,".csv"),header=T,stringsAsFactors=F)
system(paste0("gunzip ",paste0(path, expName,".idmatch.gz") ))
FragID<-read.table(paste0(path, expName,".idmatch"),header=F,stringsAsFactors=F,sep="\t")
...
minDistance<-function(v1,v2,v3,v4) {
  ...
  #it takes four vectors representin AlnStart, AlnEnd, lead(AlnStart), lead(AlnEnd)
...
fixOverlapOri<-function(df) {
  ...
  # overlap.
  # outut is vector with orientations
...
hops$hopSize[hops$hopType=="Inter"]<-NA
hops$sameStrand[hops$hopType=="Inter"]<-NA
hops$hopOri[hops$hopType!="Intra"]<-NA
hops$hopOri[is.na(hops$hopType)]<-NA

# correct hop orientation when fragments overlap
df<-hops %>%
  mutate(AlnStart2=lead(AlnStart)) %>%
  select(overlap,sameStrand,AlnStrand,AlnStart,AlnStart2,hopOri,ReadId)
hops$hopOri<-fixOverlapOri(df)

# remove an reads that do not have any contacts
readsWithHops<-hops$ReadId[!is.na(hops$hopType)]
idx<-hops$ReadId %in% readsWithHops
hopReads<-hops[idx,]
# save table
write.csv(hopReads,paste0(path,expName,"_readsWithContacts.csv"),row.names=F)

# keep only the contact information ()
contacts<-hopReads[!is.na(hopReads$hopType),]
# save table
write.csv(contacts,paste0(path,expName,"_contacts.csv"),row.names=F)
